...
void commandConectar(std::string strNumberX, std::string antennaId, std::string startMin, std::vector<std::string> vec, System* system)
{
    strNumberX = vec[1];
    antennaId = vec[2];
    startMin = vec[3];
...
void commandDesconectar(std::string strNumberX, std::vector<std::string> vec, std::string antennaId, System* system)
{
strNumberX = vec[1];
// Divide el id de la antena separando la "A" del identificador
std::vector<std::string> antennaIdV = split(antennaId, 'A');
